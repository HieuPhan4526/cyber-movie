...
export default function MovieManager() {
  ...
  data = movieList.map((movie, index) => {
    return {
      ...
      key: movie.maPhim,
      image: (
        <img style={{ width: 70, height: 70 }} src={movie.hinhAnh} alt="" />
      ),
      name: movie.tenPhim,
      description:
        movie.moTa.length > 60 ? movie.moTa.slice(0, 60) + "..." : movie.moTa,
      status: [
        `Rate: ${movie.danhGia}`,
        movie.hot ? "Hot" : "",
        movie.dangChieu ? "Đang Chiếu" : "",
        movie.sapChieu ? "Sắp Chiếu" : "",
      ],
      action: (
        <Space size="small">
          ...
          <Button
            onClick={() => {
              if (
                window.confirm(
                  "Bạn có chắc muốn xóa phim :" + movie.tenPhim + " ?"
                )
              )
          ...
          <NavLink
            to={`/admin/film/showtime/${movie.maPhim}`}
            onClick={() => {
              localStorage.setItem("movie", JSON.stringify(movie));
            }}
          >
            <Button type="text">
              <ScheduleOutlined style={{ fontSize: 25, color: "green" }} />
            </Button>
          </NavLink>
        </Space>
      ),
